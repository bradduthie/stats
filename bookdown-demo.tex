...
\protect\hyperlink{Chapter_1}{Chapter 1} will review some background mathematics that is relevant the statistical techniques that you will learn in this module.
...
While the theory underlying statistical techniques does rely on some quite advanced mathematics \citep[e.g., see][]{Mclean1991, Rencher2000, Miller2004}, applying standard statistical tools to actual data usually does not.
...
For doing statistics, what you really \emph{need} to know here are is the notation; that is, how operations such as addition, multiplication, and exponents are represented mathematically.
...
We know that the number 6 is greater than 5, and express this mathematically as the \emph{inequality}, \(6 > 5\).
...
There are multiple ways of thinking about why this is true \citep[see, e.g.,][ for a brief explanation]{Askey1999}, but for now we can take it as a given.
...
Consequently, we can say that \(\sqrt{2}\) is \emph{irrational} (not in the sense of being illogical or unreasonable, but in the sense that it cannot be represented as a ratio of two integers).
...
For example, suppose that we wanted to illustrate the change in global CO\(_{2}\) over time \citep{Friedlingstein2022}.
...
The \emph{natural logarithm} uses Euler's number, \(e \approx 2.718282\), as a base.
Euler's number \(e\) is an irrational number, like \(\pi\), which corresponds to the intrinsic rate of increase of population size in ecology \citep{Gotelli2001}, or, in banking, interest compounded continually (\(e\) actually shows up in a lot of different places throughout science and mathematics).
...
\begin{figure}
\includegraphics[width=1\linewidth]{img/handwritten_data} \caption{A portion of a lab notebook used to record measurements of fig fruits from different trees in 2010.}\label{fig:unnamed-chunk-5}
\end{figure}

Fortunately, there are some generally agreed upon guidelines for formatting data for statistical analysis.
This chapter introduces the tidy format for organising data \citep{Wickham2014}, which can be used for structuring data files to be used in statistical software.
...
Excel and \href{https://docs.google.com/spreadsheets}{Google Sheets} are also fine to use.
...
There are a lot of potential ways that data can be organised in a spreadsheet.
For good statistical analysis, there are a few principles that are helpful to follow.
When collecting data, we record observations on different units.
For example, we might make measurements on different trees, or different patches of land, or different samples of soil.
In this case, our trees, land patches, or soil samples are our units of \textbf{observation}.
...
Tidy datasets that can be used in statistical analysis are defined by three characteristics \citep{Wickham2014}.
...
If, for example, we were to measure the heights and seed numbers for 4 trees, we might organise the data as in Table 2.1.

\begin{longtable}[]{@{}llll@{}}
\caption{Hypothetical tidy data set in which each column of data is a variable and each row of data is an observational unit (tree).}\tabularnewline
\toprule
...
In contrast, variables that are actually measured tend to be in the right-most columns (e.g., tree height or seed number).
...
\textbf{Binary files} are different from text files and typically contain information besides just plain text.
...
Remember that each row will be a unique observation; in this case, a unique fig fruit from which observations were recorded.
...
\tightlist
\item
  It is important to start in column A and row 1; do not leave any empty rows or columns because when we get to the statistical analysis in Jamovi, Jamovi will assume that empty rows and columns signify missing data.
\item
...
\item
  Missing information, such as the empty boxes for the fruit dimensions in row 4 in the notebook (Figure 3.2) should be indicated with an `NA' (without the quotes). This will let Jamovi know that these data are missing.
\item
...
\begin{figure}
\includegraphics[width=1\linewidth]{img/Ch1_Ex1_seeds2} \caption{Tallies of seed counts collected from 4 fig fruits in Baja, Mexico in 2010.}\label{fig:unnamed-chunk-12}
\end{figure}
...
Here we will work with a dataset that includes counts of the number of eggs collected from female fig wasps, which are small species of insects that lay their eggs into the ovules of fig flowers \citep{Weiblen2002}.
...
How many columns did you need to create the new dataset? \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

Are there any missing data in this dataset? \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
...
Exercise 3 is, like Exercise 2, self-guided.
...
Rows list all of the species and columns list the fruits, with the counts in the middle, which is an efficient way to present the data so that they are all easy to see, but this will not work for running statistical analysis.

\begin{longtable}[]{@{}lrrrrr@{}}
\caption{\label{tab:unnamed-chunk-14}An efficienty but untidy way to present count data. Counts of different species of fig wasps (rows) in 5 different fig fruits (columns). Data were originally collected from Baja, Mexico in 2010.}\tabularnewline
\toprule
...
This problem might be a bit more challenging than Exercise 2.
...
Next comes a slightly more tricky part, which will make use of some of the background mathematics reviewed in \protect\hyperlink{Chapter_1}{Chapter 1}.
...
By typing in the sign \texttt{=}, the spreadsheet will know to start a new calculation or function in that cell.
Try this with an empty cell by typing \texttt{=\ 5\ +\ 4} in it.
...
To calculate \(V_{head}\) for the first wasp in row 2 of Figure 3.8, we could select the spreadsheet cell H2 and type the code, \texttt{=(4/3)*(3.14)*((B2+C2)/2)\^{}3}.
...
We want to add head length and width before dividing by two, so we type \texttt{((B2+C2)/2)} to ensure with the innermost parentheses that head length and width are added before dividing.
Once all of this is completed, we raise everything in parentheses to the third power using the \texttt{\^{}3}, so \texttt{((B2+C2)/2)\^{}3}.
In general, the different mathematical operations can be carried out using the the symbols in Table 3.2.
...
What are the units of total wasp volume? \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
...
\begin{figure}
\includegraphics[width=1\linewidth]{bookdown-demo_files/figure-latex/unnamed-chunk-31-1} \caption{Example histogram fig fruit width (cm) using data from 78 fig fruits collected in in 2010 from Baja, Mexico.}\label{fig:unnamed-chunk-31}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{bookdown-demo_files/figure-latex/unnamed-chunk-32-1} \caption{Example bar plot showing how many fruits were collected from each of 4 trees (78 collected in total) in 2010 from Baja, Mexico.}\label{fig:unnamed-chunk-32}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{bookdown-demo_files/figure-latex/unnamed-chunk-33-1} \caption{Example pie plot showing the percentage of fruits that were collected from each of 4 trees (78 collected in total) in 2010 from Baja, Mexico.}\label{fig:unnamed-chunk-33}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{bookdown-demo_files/figure-latex/unnamed-chunk-34-1} \caption{Boxplot (A) of fig fruit widths (cm) for 78 fig fruits collected in 2010 in Baja, Mexico. Panel (B) presents the same data as a histogram.}\label{fig:unnamed-chunk-34}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{bookdown-demo_files/figure-latex/unnamed-chunk-37-1} \caption{Example histogram fig fruit width (cm) using data from 78 fig fruits collected in in 2010 from Baja, Mexico.}\label{fig:unnamed-chunk-37}
\end{figure}
...
\toprule
\endhead
Sample\_1 & 0.68 & 0.92 & 0.61 & 0.69 & 0.75 & 0.75 & 0.76 & 0.55 & 0.80 & 0.70 \\
Sample\_2 & 0.72 & 0.90 & 0.78 & 0.80 & 0.69 & 0.84 & 0.82 & 0.63 & 0.65 & 0.80 \\
Sample\_3 & 0.64 & 0.76 & 0.69 & 0.63 & 0.76 & 0.58 & 0.66 & 0.57 & 0.65 & 0.55 \\
Sample\_4 & 0.83 & 0.65 & 0.75 & 0.65 & 0.67 & 0.73 & 0.70 & 0.62 & 0.75 & 0.63 \\
Sample\_5 & 0.74 & 0.47 & 0.75 & 0.62 & 0.68 & 0.61 & 0.71 & 0.73 & 0.92 & 0.56 \\
Sample\_6 & 0.75 & 0.70 & 0.74 & 0.54 & 0.58 & 0.71 & 0.64 & 0.66 & 0.63 & 0.89 \\
Sample\_7 & 0.67 & 0.76 & 0.64 & 0.82 & 0.76 & 0.93 & 0.71 & 0.68 & 0.77 & 0.62 \\
Sample\_8 & 0.52 & 0.86 & 0.85 & 0.85 & 0.75 & 0.77 & 0.69 & 0.71 & 0.90 & 0.85 \\
Sample\_9 & 0.73 & 0.68 & 0.69 & 0.61 & 0.86 & 0.61 & 0.69 & 0.69 & 0.66 & 0.77 \\
Sample\_10 & 0.82 & 0.73 & 0.59 & 0.60 & 0.70 & 0.79 & 0.56 & 0.69 & 0.74 & 0.70 \\
Sample\_11 & 0.85 & 0.66 & 0.68 & 0.69 & 0.74 & 0.67 & 0.66 & 0.59 & 0.78 & 0.65 \\
Sample\_12 & 0.97 & 0.60 & 0.65 & 0.70 & 0.75 & 0.60 & 0.84 & 0.73 & 0.61 & 0.75 \\
Sample\_13 & 0.86 & 0.72 & 0.61 & 0.73 & 0.69 & 0.71 & 0.76 & 0.74 & 0.59 & 0.61 \\
Sample\_14 & 0.65 & 0.76 & 0.90 & 0.69 & 0.89 & 0.65 & 0.59 & 0.91 & 0.83 & 0.83 \\
Sample\_15 & 0.89 & 0.67 & 0.72 & 0.69 & 0.67 & 0.87 & 0.89 & 0.84 & 0.71 & 0.78 \\
Sample\_16 & 0.61 & 0.65 & 0.75 & 0.80 & 0.70 & 0.70 & 0.79 & 0.65 & 0.67 & 0.66 \\
Sample\_17 & 0.83 & 0.71 & 0.79 & 0.59 & 0.68 & 0.80 & 0.66 & 0.74 & 0.75 & 0.83 \\
Sample\_18 & 0.65 & 0.70 & 0.79 & 0.75 & 0.72 & 0.66 & 0.50 & 0.77 & 0.68 & 0.64 \\
Sample\_19 & 0.64 & 0.70 & 0.61 & 0.72 & 0.84 & 0.55 & 0.71 & 0.81 & 0.69 & 0.72 \\
Sample\_20 & 0.70 & 0.83 & 0.69 & 0.79 & 0.63 & 0.58 & 0.72 & 0.74 & 0.92 & 0.50 \\
\bottomrule
\end{longtable}
...
## [1,] 0.721 0.763 0.649 0.698 0.679 0.684 0.736 0.775 0.699 0.692
## [2,] 0.697 0.720 0.702 0.770 0.773 0.698 0.738 0.686 0.699 0.710
\end{verbatim}

The standard deviation of the sample means reported above is 0.0347237.
Now suppose that we only had Sample 1 (i.e., the top row of data).
The standard deviation of Sample 1 is \(s =\) 0.1020294.
We can calculate the standard error from these sample values below,

\[s = \frac{0.1020294}{\sqrt{10}} = 0.0322645.\]

The estimate of the standard error from calculating the standard deviation of the sample means is therefore 0.0347237, and the estimate from just using the standard error formula and data from only Sample 1 is 0.0322645.
...
\begin{figure}
\includegraphics[width=1\linewidth]{img/Nymphaea_alba_pads} \caption{Photo of white water lillies on the water.}\label{fig:unnamed-chunk-42}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{img/lilypad_tidy} \caption{Tidying the raw data of petiole diameters from lily pad measurements across 7 sites in Scotland. A new column of data is created by right clicking on an existing column and choosing 'Add Variable'.}\label{fig:unnamed-chunk-44}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{img/lilypad_descriptives} \caption{Jamovi toolbar after having selected on the Analyses tab followed by the Exploration button.}\label{fig:unnamed-chunk-46}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{img/cards} \caption{Playing cards can be useful for illustrating concepts in probability. Here we have 5 hearts (left) and 5 clubs (right).}\label{fig:unnamed-chunk-51}
\end{figure}
...
\begin{figure}
\includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-58-1} \caption{A simulated demonstration of the central limit theorem. (a) Recreation of Figure 14.6 showing the probability distribution for the number of people who have Covid-19 in a shop of 6 when the probability of testing positive is 0.025. (b) The distribution of 1000 means sampled from panel (a), where the sample size is 100.}\label{fig:unnamed-chunk-58}
\end{figure}
...
\begin{figure}
\includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-59-1} \caption{A simulated demonstration of the central limit theorem. (a) Recreation of Figure 14.7 showing the probability distribution for the number of events occurring in a poisson distribution with a rate parameter of 1. (b) The distribution of 1000 means sampled from panel (a), where the sample size is 1000.}\label{fig:unnamed-chunk-59}
\end{figure}
...
\begin{figure}
\includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-60-1} \caption{A simulated demonstration of the central limit theorem. (a) Recreation of Figure 14.8 showing a continuous uniform distribution with a minimum of 1 and a maximum of 2.5. (b) The distribution of 1000 means sampled from panel (a), where the sample size is 10000.}\label{fig:unnamed-chunk-60}
\end{figure}
...
\begin{figure}
\includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-61-1} \caption{A normal distribution in which the shaded region shows the area within one standard deviation of the mean (dotted line); that is, the shaded region starts on the left at the mean minus one standard deviation, then ends at the right at the mean plus one standard deviation. This shaded area encompases 68.2 per cent of the total area under the curve.}\label{fig:unnamed-chunk-61}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{img/power_up} \caption{Welcome screen of the mobile game Power Up!}\label{fig:unnamed-chunk-65}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{bookdown-demo_files/figure-latex/unnamed-chunk-68-1} \caption{Distribution of player scores in the game Power Up!}\label{fig:unnamed-chunk-68}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{bookdown-demo_files/figure-latex/unnamed-chunk-69-1} \caption{Distribution of player scores in the game Power Up! shown in histogram bars. The overlaid curve shows the probability density function for a normal distribution that has the same mean and standard deviation as the sample described by the histogram.}\label{fig:unnamed-chunk-69}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{img/jamovi_toolbar_modules} \caption{Jamovi tool bar, which includes an option for downloading new Modules (right hand side)}\label{fig:unnamed-chunk-70}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{img/jamovi_toolbar_modules_distrACTION} \caption{Jamovi tool bar, which includes an added module called distrACTION.}\label{fig:unnamed-chunk-71}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{img/jamovi_normal_distribution} \caption{Jamovi options for the distrACTION module for computing probability for a given normal distribution. The example shown here calculates the probability that a value sampled from the normal distribution of interest is less than 80.}\label{fig:unnamed-chunk-72}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{img/jamovi_RjEditor} \caption{Jamovi interface for the Rj Editor module. Code can be run by clicking on the green triangle in the upper left.}\label{fig:unnamed-chunk-74}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{img/jamovi_clt} \caption{Jamovi interface for the 'Demonstrations' module, which allows users to randomly generate data from a specific source distribution (normal, uniform, geometric, lognormal, and binary), sample size, and number of trials (i.e., variables)}\label{fig:unnamed-chunk-77}
\end{figure}
