...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Visualising the z-scores and p-values"),
    
    hr(),
    h4("The plot below shows the standard normal distribution (i.e., mean = 0 and standard deviation = 1). Use the slider at the bottom to change the z-score in the histogram, and see how the probability of sampling a value above and below the z-score changes. Use the pull down menu to switch to a two-tailed distribution to see the probability of sampling a value as more extreme than your z-score in either direction."),
    hr(),
    
    plotOutput("distPlot"),
    
    fluidRow(
        sliderInput("z", label = "z", width="100%", min = -4, 
    ...
    output$distPlot <- renderPlot({
        if(input$tailed == "1"){
            ...
            plot(x = xx, y = yy, type = "l", lwd = 4, cex.lab = 2, cex.lab = 2,
                 cex.axis = 2, ylab = "Probability density",
                 xlab = "z-score (distance from the mean)");
            ...
            pbrown   <- round(1 - pnorm(q = zz), digits = 3);
            ...
            if(zz >= 0){
                ...
                text(x = mean(c(-5, -2.5)), y = mean(c(0.15, 0.2)), 
            ...
            text(x = mean(c(5, 2.5)), y = 0.325, labels = "Probability brown",
                 cex = 1.5);
            polygon(x=tbox$x, y=tbox$y, lwd=3, border="black", col="#E69F00");
            pbrown   <- 2 * round(1 - pnorm(q = zz), digits = 3);
            pblue    <- round(1 - pbrown, digits = 3);
            text(x = mean(c(-5, -2.5)), y = mean(c(0.15, 0.3)), 
                 labels = pblue, cex = 2);
            text(x = mean(c(5, 2.5)), y = mean(c(0.15, 0.3)), 
